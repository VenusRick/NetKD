"""
Teacher models and stacking ensemble for SD-MKD.

This module defines torchvision-based teachers, along with training
routines for individual teachers (Phase I) and the stacking ensemble (Phase II).

Supported Teachers:
- ResNet50Teacher: ResNet-50 with optional ECA attention
- DenseNet121Teacher: DenseNet-121 with optional ECA attention
- MobileNetV3LargeTeacher: MobileNetV3-Large with optional ECA attention
- ConvNeXtV2TinyTeacher: ConvNeXtV2-Tiny with optional ECA attention (NEW)
- EfficientNetV2STeacher: EfficientNetV2-S with optional ECA attention (NEW)
"""

from __future__ import annotations

from dataclasses import dataclass
from typing import Iterable, Tuple, Optional, List, Dict, Any

import torch
import torch.nn as nn
import torch.nn.functional as F
import torchvision.models as models
from torch.utils.data import DataLoader

try:
    from .eca_module import add_eca_to_densenet_v2, ECALayer
except ImportError:
    ECALayer = None


def _adjust_first_conv(module: nn.Module, in_channels: int = 1):
    """Replace the first convolution to accept a different number of channels."""

    if isinstance(module, models.ResNet):
        conv = module.conv1
        new_conv = nn.Conv2d(in_channels, conv.out_channels, conv.kernel_size, conv.stride, conv.padding, bias=False)
        if conv.weight.shape[1] == 3:
            new_conv.weight.data = conv.weight.data.mean(dim=1, keepdim=True)
        module.conv1 = new_conv
    elif isinstance(module, models.DenseNet):
        conv = module.features.conv0
        new_conv = nn.Conv2d(in_channels, conv.out_channels, conv.kernel_size, conv.stride, conv.padding, bias=False)
        if conv.weight.shape[1] == 3:
            new_conv.weight.data = conv.weight.data.mean(dim=1, keepdim=True)
        module.features.conv0 = new_conv
    elif isinstance(module, models.MobileNetV3):
        conv = module.features[0][0]
        new_conv = nn.Conv2d(in_channels, conv.out_channels, conv.kernel_size, conv.stride, conv.padding, bias=False)
        if conv.weight.shape[1] == 3:
            new_conv.weight.data = conv.weight.data.mean(dim=1, keepdim=True)
        module.features[0][0] = new_conv
    elif hasattr(models, 'ConvNeXt') and isinstance(module, models.ConvNeXt):
        # ConvNeXt stem: features[0][0] is the first conv
        conv = module.features[0][0]
        new_conv = nn.Conv2d(in_channels, conv.out_channels, conv.kernel_size, conv.stride, conv.padding, bias=conv.bias is not None)
        if conv.weight.shape[1] == 3:
            new_conv.weight.data = conv.weight.data.mean(dim=1, keepdim=True)
        if conv.bias is not None:
            new_conv.bias.data = conv.bias.data
        module.features[0][0] = new_conv
    elif hasattr(models, 'EfficientNet') and isinstance(module, models.EfficientNet):
        # EfficientNet stem: features[0][0] is the first conv
        conv = module.features[0][0]
        new_conv = nn.Conv2d(in_channels, conv.out_channels, conv.kernel_size, conv.stride, conv.padding, bias=False)
        if conv.weight.shape[1] == 3:
            new_conv.weight.data = conv.weight.data.mean(dim=1, keepdim=True)
        module.features[0][0] = new_conv
    else:
        raise ValueError(f"Unsupported model type: {type(module)}")


class ResNet50Teacher(nn.Module):
    def __init__(self, num_classes: int, pretrained: bool = False, use_eca: bool = False):
        super().__init__()
        base = models.resnet50(weights=models.ResNet50_Weights.IMAGENET1K_V1 if pretrained else None)
        _adjust_first_conv(base, in_channels=1)
        in_dim = base.fc.in_features
        base.fc = nn.Linear(in_dim, num_classes)
        
        if use_eca and ECALayer is not None:
            for layer_name in ['layer1', 'layer2', 'layer3', 'layer4']:
                layer = getattr(base, layer_name)
                for block in layer:
                    if hasattr(block, 'conv3'):
                        out_ch = block.conv3.out_channels
                        eca_layer = ECALayer(out_ch)
                        setattr(block, 'eca', eca_layer)
        
        self.model = base

    def forward(self, x):
        out = self.model.conv1(x)
        out = self.model.bn1(out)
        out = self.model.relu(out)
        out = self.model.maxpool(out)
        
        for layer_name in ['layer1', 'layer2', 'layer3', 'layer4']:
            layer = getattr(self.model, layer_name)
            for block in layer:
                identity = out
                out = block.conv1(out)
                out = block.bn1(out)
                out = block.relu(out)
                out = block.conv2(out)
                out = block.bn2(out)
                out = block.relu(out)
                out = block.conv3(out)
                out = block.bn3(out)
                if hasattr(block, 'eca'):
                    out = block.eca(out)
                if block.downsample is not None:
                    identity = block.downsample(identity)
                out += identity
                out = block.relu(out)
        
        out = self.model.avgpool(out)
        out = torch.flatten(out, 1)
        out = self.model.fc(out)
        return out


class MobileNetV3LargeTeacher(nn.Module):
    def __init__(self, num_classes: int, pretrained: bool = False, use_eca: bool = False):
        super().__init__()
        base = models.mobilenet_v3_large(weights=models.MobileNet_V3_Large_Weights.IMAGENET1K_V1 if pretrained else None)
        _adjust_first_conv(base, in_channels=1)
        in_dim = base.classifier[-1].in_features
        base.classifier[-1] = nn.Linear(in_dim, num_classes)
        
        if use_eca and ECALayer is not None:
            for block in base.features:
                if hasattr(block, 'block') and len(block.block) >= 3:
                    for layer in reversed(list(block.block)):
                        if isinstance(layer, nn.Conv2d):
                            setattr(block, 'eca', ECALayer(layer.out_channels))
                            break
        
        self.model = base

    def forward(self, x):
        return self.model(x)


class DenseNet121Teacher(nn.Module):
    def __init__(self, num_classes: int, pretrained: bool = False, use_eca: bool = False):
        super().__init__()
        base = models.densenet121(weights=models.DenseNet121_Weights.IMAGENET1K_V1 if pretrained else None)
        _adjust_first_conv(base, in_channels=1)
        in_dim = base.classifier.in_features
        base.classifier = nn.Linear(in_dim, num_classes)
        
        if use_eca and ECALayer is not None:
            # Prefer the robust insertion that handles DenseNet internals.
            try:
                from .eca_module import add_eca_to_densenet_v2
                base = add_eca_to_densenet_v2(base)
            except Exception:
                # Fallback: best-effort attach ECA to dense blocks
                for name, module in base.features.named_children():
                    if 'denseblock' in name:
                        num_layers = len(list(module.children()))
                        if num_layers > 0:
                            first_layer = list(module.children())[0]
                            if hasattr(first_layer, 'conv2'):
                                growth_rate = first_layer.conv2.out_channels
                                total_channels = 64 + num_layers * growth_rate
                                setattr(module, 'eca', ECALayer(total_channels))

        self.model = base

    def forward(self, x):
        return self.model(x)


class ConvNeXtV2TinyTeacher(nn.Module):
    """
    ConvNeXtV2-Tiny Teacher Model with optional ECA attention.
    
    ConvNeXtV2 uses FCMAE (Fully Convolutional Masked Autoencoder) pretraining,
    which is particularly effective for smaller datasets.
    
    Architecture:
    - 4 stages with depths [3, 3, 9, 3]
    - Channel dimensions [96, 192, 384, 768]
    - Uses Global Response Normalization (GRN)
    """
    
    def __init__(self, num_classes: int, pretrained: bool = False, use_eca: bool = False):
        super().__init__()
        
        # Use torchvision ConvNeXt-Tiny for compatibility with existing checkpoints
        base = models.convnext_tiny(weights=models.ConvNeXt_Tiny_Weights.IMAGENET1K_V1 if pretrained else None)
        _adjust_first_conv(base, in_channels=1)
        
        # Replace classifier
        in_dim = base.classifier[-1].in_features
        base.classifier[-1] = nn.Linear(in_dim, num_classes)
        
        self._use_timm = False
        
        # Add ECA attention to ConvNeXt blocks
        if use_eca and ECALayer is not None:
            self._add_eca_to_convnext(base)
        
        self.model = base
    
    def _add_eca_to_convnext(self, model):
        """Add ECA layers after each ConvNeXt block."""
        if hasattr(model, 'stages'):
            # timm model structure
            for stage in model.stages:
                if hasattr(stage, 'blocks'):
                    for block in stage.blocks:
                        if hasattr(block, 'conv_dw'):
                            out_ch = block.conv_dw.out_channels
                            setattr(block, 'eca', ECALayer(out_ch))
        elif hasattr(model, 'features'):
            # torchvision model structure
            for i, layer in enumerate(model.features):
                if hasattr(layer, '__len__'):
                    for j, block in enumerate(layer):
                        if hasattr(block, 'block'):
                            # CNBlock: block contains [dwconv, layernorm, linear, ...]
                            for k, sublayer in enumerate(block.block):
                                if isinstance(sublayer, nn.Conv2d):
                                    out_ch = sublayer.out_channels
                                    setattr(block, 'eca', ECALayer(out_ch))
                                    break
    
    def forward(self, x):
        return self.model(x)


class EfficientNetV2STeacher(nn.Module):
    """
    EfficientNetV2-S Teacher Model with optional ECA attention.
    
    EfficientNetV2 introduces:
    - Progressive learning with adaptive regularization
    - Fused-MBConv blocks in early stages
    - Faster training and better accuracy
    
    Architecture:
    - Uses Fused-MBConv in early layers
    - Uses MBConv in later layers
    - Compound scaling with improved efficiency
    """
    
    def __init__(self, num_classes: int, pretrained: bool = False, use_eca: bool = False):
        super().__init__()
        
        base = models.efficientnet_v2_s(
            weights=models.EfficientNet_V2_S_Weights.IMAGENET1K_V1 if pretrained else None
        )
        _adjust_first_conv(base, in_channels=1)
        
        # Replace classifier
        in_dim = base.classifier[-1].in_features
        base.classifier[-1] = nn.Linear(in_dim, num_classes)
        
        # Add ECA attention to MBConv/FusedMBConv blocks
        if use_eca and ECALayer is not None:
            self._add_eca_to_efficientnet(base)
        
        self.model = base
    
    def _add_eca_to_efficientnet(self, model):
        """Add ECA layers after each MBConv/FusedMBConv block."""
        if hasattr(model, 'features'):
            for i, block in enumerate(model.features):
                # Skip first conv and last conv
                if i == 0 or i == len(model.features) - 1:
                    continue
                
                # MBConv and FusedMBConv blocks are Sequential
                if hasattr(block, '__len__') and len(block) > 0:
                    for j, subblock in enumerate(block):
                        if hasattr(subblock, 'block'):
                            # Find the expansion conv or depthwise conv
                            for layer in reversed(list(subblock.block)):
                                if isinstance(layer, nn.Conv2d):
                                    out_ch = layer.out_channels
                                    setattr(subblock, 'eca', ECALayer(out_ch))
                                    break
    
    def forward(self, x):
        return self.model(x)


# =============================================================================
# Legacy Ensemble (for backward compatibility)
# =============================================================================

class TeacherEnsemble(nn.Module):
    """Legacy 3-teacher ensemble for backward compatibility."""
    
    def __init__(self, num_classes: int, pretrained: bool = False):
        super().__init__()
        self.t1 = ResNet50Teacher(num_classes, pretrained)
        self.t2 = MobileNetV3LargeTeacher(num_classes, pretrained)
        self.t3 = DenseNet121Teacher(num_classes, pretrained)

    @torch.no_grad()
    def forward_all(self, x) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        l1 = self.t1(x)
        l2 = self.t2(x)
        l3 = self.t3(x)
        return l1, l2, l3


class StackingModel(nn.Module):
    """Legacy stacking model for 3 teachers."""
    
    def __init__(self, num_classes: int, hidden_dim: int = 384):
        super().__init__()
        in_dim = 3 * num_classes
        self.mlp = nn.Sequential(
            nn.Linear(in_dim, hidden_dim),
            nn.ReLU(inplace=True),
            nn.Linear(hidden_dim, num_classes),
        )

    def forward(self, logits1, logits2, logits3):
        x = torch.cat([logits1, logits2, logits3], dim=-1)
        return self.mlp(x)


# =============================================================================
# Dynamic Ensemble (new, supports variable number of teachers)
# =============================================================================

class DynamicTeacherEnsemble(nn.Module):
    """
    Dynamic ensemble supporting arbitrary number of teachers.
    
    Unlike TeacherEnsemble which is hardcoded for 3 teachers,
    this class can work with any number of teachers.
    """
    
    def __init__(self, teachers: Dict[str, nn.Module]):
        """
        Initialize with a dictionary of teachers.
        
        Args:
            teachers: Dict mapping teacher names to model instances
        """
        super().__init__()
        self.teacher_names = list(teachers.keys())
        
        # Register teachers as submodules
        for name, model in teachers.items():
            self.add_module(name, model)
    
    @property
    def num_teachers(self) -> int:
        return len(self.teacher_names)
    
    def get_teacher(self, name: str) -> nn.Module:
        return getattr(self, name)
    
    @torch.no_grad()
    def forward_all(self, x) -> List[torch.Tensor]:
        """Get logits from all teachers."""
        return [getattr(self, name)(x) for name in self.teacher_names]
    
    def forward(self, x) -> torch.Tensor:
        """Average ensemble prediction."""
        logits_list = self.forward_all(x)
        return torch.stack(logits_list, dim=0).mean(dim=0)


class DynamicStackingModel(nn.Module):
    """
    Dynamic stacking model supporting arbitrary number of teachers.
    
    Features:
    - Variable input dimension based on number of teachers
    - Optional dropout for regularization
    - Deeper MLP with residual-style connections
    """
    
    def __init__(
        self,
        num_classes: int,
        num_teachers: int,
        hidden_dim: int = 384,
        dropout: float = 0.1,
    ):
        super().__init__()
        self.num_classes = num_classes
        self.num_teachers = num_teachers
        
        in_dim = num_teachers * num_classes
        
        self.mlp = nn.Sequential(
            nn.Linear(in_dim, hidden_dim),
            nn.ReLU(inplace=True),
            nn.Dropout(dropout),
            nn.Linear(hidden_dim, hidden_dim // 2),
            nn.ReLU(inplace=True),
            nn.Dropout(dropout),
            nn.Linear(hidden_dim // 2, num_classes),
        )
    
    def forward(self, *teacher_logits: torch.Tensor) -> torch.Tensor:
        """
        Forward pass.
        
        Args:
            *teacher_logits: Variable number of teacher logit tensors
            
        Returns:
            Fused logits
        """
        if len(teacher_logits) != self.num_teachers:
            raise ValueError(
                f"Expected {self.num_teachers} teacher logits, got {len(teacher_logits)}"
            )
        x = torch.cat(teacher_logits, dim=-1)
        return self.mlp(x)


# ---------------------------------------------------------------------------
# Training helpers
# ---------------------------------------------------------------------------

@dataclass
class TrainResult:
    epoch: int
    train_loss: float
    val_loss: float
    val_acc: float


def _evaluate(model, loader: DataLoader, device: torch.device) -> Tuple[float, float]:
    """
    Evaluate a model (nn.Module) or callable on a data loader.
    
    Args:
        model: Either an nn.Module or a callable that takes input and returns logits
        loader: DataLoader to evaluate on
        device: Device to run evaluation on
        
    Returns:
        Tuple of (val_loss, accuracy)
    """
    # Set to eval mode if it's a module
    if isinstance(model, nn.Module):
        model.eval()
    
    total = 0
    correct = 0
    losses = []
    criterion = nn.CrossEntropyLoss()
    with torch.no_grad():
        for x, y in loader:
            x, y = x.to(device), y.to(device)
            logits = model(x)
            loss = criterion(logits, y)
            losses.append(loss.item())
            preds = logits.argmax(dim=-1)
            total += y.numel()
            correct += (preds == y).sum().item()
    val_loss = float(sum(losses) / max(1, len(losses)))
    acc = correct / max(1, total)
    return val_loss, acc


def train_single_teacher(
    model: nn.Module,
    train_loader: DataLoader,
    val_loader: DataLoader,
    device: torch.device,
    epochs: int = 1,
    lr: float = 1e-3,
    weight_decay: float = 1e-4,
    optimizer_name: str = "adamw",
    class_weights: torch.Tensor | None = None,
    label_smoothing: float = 0.05,
    scheduler_patience: int = 2,
    scheduler_factor: float = 0.5,
    min_lr: float = 1e-6,
) -> Iterable[TrainResult]:
    """Train a single teacher using cross-entropy."""

    model.to(device)
    weight = class_weights.to(device) if class_weights is not None else None
    criterion = nn.CrossEntropyLoss(weight=weight, label_smoothing=label_smoothing)
    if optimizer_name.lower() == "sgd":
        optimizer = torch.optim.SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=weight_decay)
    else:
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=weight_decay)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode="min", factor=scheduler_factor, patience=scheduler_patience, min_lr=min_lr
    )

    for epoch in range(1, epochs + 1):
        model.train()
        epoch_losses = []
        for x, y in train_loader:
            x, y = x.to(device), y.to(device)
            optimizer.zero_grad()
            logits = model(x)
            loss = criterion(logits, y)
            loss.backward()
            optimizer.step()
            epoch_losses.append(loss.item())

        train_loss = float(sum(epoch_losses) / max(1, len(epoch_losses)))
        val_loss, val_acc = _evaluate(model, val_loader, device)
        scheduler.step(val_loss)
        yield TrainResult(epoch, train_loss, val_loss, val_acc)


def train_stacking_model(
    ensemble: TeacherEnsemble,
    stacking_model: StackingModel,
    train_loader: DataLoader,
    val_loader: DataLoader,
    device: torch.device,
    epochs: int = 1,
    lr: float = 1e-3,
    weight_decay: float = 1e-4,
) -> Iterable[TrainResult]:
    """Train the stacking model on frozen teachers."""

    ensemble.eval()
    for p in ensemble.parameters():
        p.requires_grad_(False)
    ensemble.to(device)
    stacking_model.to(device)

    optimizer = torch.optim.AdamW(stacking_model.parameters(), lr=lr, weight_decay=weight_decay)
    criterion = nn.CrossEntropyLoss()

    for epoch in range(1, epochs + 1):
        stacking_model.train()
        epoch_losses = []
        for x, y in train_loader:
            x, y = x.to(device), y.to(device)
            with torch.no_grad():
                l1, l2, l3 = ensemble.forward_all(x)
            logits = stacking_model(l1, l2, l3)
            loss = criterion(logits, y)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            epoch_losses.append(loss.item())

        train_loss = float(sum(epoch_losses) / max(1, len(epoch_losses)))
        val_loss, val_acc = _evaluate(lambda z: stacking_model(*ensemble.forward_all(z)), val_loader, device)
        yield TrainResult(epoch, train_loss, val_loss, val_acc)


def train_dynamic_stacking(
    ensemble: DynamicTeacherEnsemble,
    stacking_model: DynamicStackingModel,
    train_loader: DataLoader,
    val_loader: DataLoader,
    device: torch.device,
    epochs: int = 1,
    lr: float = 1e-3,
    weight_decay: float = 1e-4,
) -> Iterable[TrainResult]:
    """Train a dynamic stacking model on frozen teachers."""

    ensemble.eval()
    for p in ensemble.parameters():
        p.requires_grad_(False)
    ensemble.to(device)
    stacking_model.to(device)

    optimizer = torch.optim.AdamW(stacking_model.parameters(), lr=lr, weight_decay=weight_decay)
    criterion = nn.CrossEntropyLoss()

    for epoch in range(1, epochs + 1):
        stacking_model.train()
        epoch_losses = []
        for x, y in train_loader:
            x, y = x.to(device), y.to(device)
            with torch.no_grad():
                logits_list = ensemble.forward_all(x)
            logits = stacking_model(*logits_list)
            loss = criterion(logits, y)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            epoch_losses.append(loss.item())

        train_loss = float(sum(epoch_losses) / max(1, len(epoch_losses)))
        
        def eval_fn(z):
            return stacking_model(*ensemble.forward_all(z))
        val_loss, val_acc = _evaluate(eval_fn, val_loader, device)
        
        yield TrainResult(epoch, train_loss, val_loss, val_acc)


__all__ = [
    # Teachers
    "ResNet50Teacher",
    "MobileNetV3LargeTeacher",
    "DenseNet121Teacher",
    "ConvNeXtV2TinyTeacher",
    "EfficientNetV2STeacher",
    # Legacy Ensemble
    "TeacherEnsemble",
    "StackingModel",
    # Dynamic Ensemble
    "DynamicTeacherEnsemble",
    "DynamicStackingModel",
    # Training
    "train_single_teacher",
    "train_stacking_model",
    "train_dynamic_stacking",
    "TrainResult",
]
