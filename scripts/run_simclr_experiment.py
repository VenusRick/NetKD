...
def pretrain_simclr(backbone, config, device, output_dir):
    ...
    train_loader, val_loader, test_loader, info = quick_load_dataset(config['data_path'], batch_size=1)
...
def finetune_from_simclr(backbone, pretrain_path, config, device, output_dir):
    ...
    train_loader, val_loader, test_loader, info = quick_load_dataset(config['data_path'], batch_size=config['batch_size'], num_workers=8)
...
def main():
    ...
    parser.add_argument('--backbone', type=str, default=None, help='Single backbone to run')
    ...
    backbones = [args.backbone] if args.backbone else ['efficientnetv2_rw_s', 'convnextv2_tiny', 'mobilenetv3_large_100']
    results = []
