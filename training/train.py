...
# Ensure the project root is on sys.path so this script works when run directly.
...
def run_real_pipeline(
    ...
    class_weights = (
        torch.tensor(meta["class_weights"], dtype=torch.float32)
        if use_class_weights and "class_weights" in meta
        else None
    )
    ...
    def _ensure_teachers() -> Dict[str, str]:
        ...
        if not force_retrain_teachers and all(Path(p).exists() for p in paths.values()):
            print("[Info] Found existing teacher checkpoints, skipping retrain.")
            return paths
...
def main():
    ...
    parser.add_argument(
        "--teacher_pretrained",
        dest="teacher_pretrained",
        action="store_true",
        help="Use ImageNet-pretrained weights for teacher backbones",
    )
    parser.add_argument(
        "--no_teacher_pretrained",
        dest="teacher_pretrained",
        action="store_false",
        help="Disable ImageNet-pretrained weights for teacher backbones",
    )
    parser.add_argument(
        "--no_class_weights",
        dest="use_class_weights",
        action="store_false",
        help="Disable inverse-frequency class weights when training teachers",
    )
    parser.add_argument(
        "--force_retrain_teachers",
        action="store_true",
        help="Ignore existing teacher checkpoints and retrain them",
    )
    parser.set_defaults(teacher_pretrained=True, use_class_weights=True)
